...
export async function GET() {
  try {
    ...
    const filteredData = studentCompletion.map((student) => {
      ...
      // Calculate average score for completed quizzes and voice exercises
      const quizScores = student.StudentQuizHistory
        .filter((q) => q.completed)
        .map((q) => q.score || 0); // Default to 0 if score is missing
      const voiceExerciseScores = student.VoiceExcercisesHistory
        .filter((v) => v.completed)
        .map((v) => v.score || 0); // Default to 0 if score is missing
      ...
      // Combine the progress and average scores into a final completion average
      ...
      return {
        ...
        totalModules,
        averageProgress: combinedCompletionAverage, // Use combined average here
      };
    });
